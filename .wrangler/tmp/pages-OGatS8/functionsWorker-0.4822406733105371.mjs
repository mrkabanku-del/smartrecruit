...
async function onRequestPut(context) {
  try {
    ...
    const updatedStage = stage !== void 0 ? stage : applicant.stage;
    const updatedName = name !== void 0 ? name : applicant.name;
    const updatedEmail = email !== void 0 ? email : applicant.email;
    const updatedPhone = phone !== void 0 ? phone : applicant.phone;
    const updatedResume = resume_url !== void 0 ? resume_url : applicant.resume_url;
    const updatedPortfolio = portfolio_url !== void 0 ? portfolio_url : applicant.portfolio_url;
    const updatedCover = cover_letter !== void 0 ? cover_letter : applicant.cover_letter;
...
async function onRequestPost2(context) {
  try {
    ...
    if (!applicant_id || !title || !scheduled_at) {
      return new Response(JSON.stringify({ error: "Applicant ID, Title, and Scheduled Date are required" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    const applicant = await context.env.DB.prepare("SELECT name FROM applicants WHERE id = ?").bind(applicant_id).first();
    if (!applicant) {
      return new Response(JSON.stringify({ error: "Applicant not found" }), {
        status: 404,
        headers: { "Content-Type": "application/json" }
    ...
    await context.env.DB.prepare(
      `INSERT INTO interviews (id, applicant_id, title, scheduled_at, duration_minutes, notes) 
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(id, applicant_id, title, scheduled_at, duration, notes || "").run();
    ...
    if (currentStage && (currentStage.stage === "new" || currentStage.stage === "screening")) {
      await context.env.DB.prepare("UPDATE applicants SET stage = ?, updated_at = ? WHERE id = ?").bind("interview", (/* @__PURE__ */ new Date()).toISOString(), applicant_id).run();
    }
    const newInterview = {
      id,
      applicant_id,
...
async function onRequestPost3(context) {
  try {
    ...
    ).bind(id, title, description || "", projectStatus).run();
    const project = { id, title, description, status: projectStatus, created_at: (/* @__PURE__ */ new Date()).toISOString() };
...
// ../.wrangler/tmp/pages-OGatS8/functionsRoutes-0.46524665789006614.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-D8pMGj/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-D8pMGj/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.4822406733105371.mjs.map
